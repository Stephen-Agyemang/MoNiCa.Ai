...
export const ProfessionalMetricChart = ({ label, value, color = 'var(--accent)' }) => {
  ...
  return (
    <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '12px 16px', borderRadius: '16px', margin: '0' }}>
      <div style={{ position: 'relative', width: '48px', height: '48px' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 800, color: '#1e293b' }}>
          {value}
        </div>
      </div>
      <div>
        <p style={{ margin: 0, fontSize: '11px', color: '#475569', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</p>
        <div style={{ height: '4px', width: '80px', background: 'rgba(0,0,0,0.06)', borderRadius: '2px', marginTop: '6px', overflow: 'hidden' }}>
